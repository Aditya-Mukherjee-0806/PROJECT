...
public class TTTAI
{
    ...
    int tools3(char play[],int m)
    {
        ...
        if(z!=-1)
            return z;
        //checks for oponent win and stops it
        z = move2(play,m);
        if(z!=-1)
            return z;
    ...
    {
        //this checks for the X in line to prevnent player from wining the game
        char ch;//allots X and O as required
        if(n==2)
            ch='X';
        else 
    ...
    int input(char play[],int n,int x,int n1)
    {
    ...
    int first(char play[],int m,int x,int n1)
    {
        ...
        {
            //if player dosen't use center spot to give a move then a confirm win
            n = 1;
            i++;
            return fwin1(play,x);
        }
        ...
        {
            //if player use centre spot and gives a move
            n=2;
            i++;
            return 6;
        }
    ...
    int second(char play[],int n,int n1)
    {
        ...
        if(i==0)//CHECKING FOR WINNING CASE
            return move1(play,n);
        else if(i==1&&play[4]=='O')//STOPING THE WINNIG CASE
        {
